...
export default function DemoContent() {
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
        ...
        <div className={css.workspace}>
          <section aria-label="Anpassa och begär en gratis demo" className={`${css.controls} ${mobilePanel !== 'settings' ? css.mobileHidden : ''}`}>
            {status === 'success' ? <div role="status" className="py-8"><span aria-hidden="true" className="inline-flex h-14 w-14 items-center justify-center rounded-full bg-accent/20 text-2xl text-accent-light">✓</span><h2 ref={heading} tabIndex={-1} className="font-display mt-6 text-3xl font-bold">Din förfrågan är skickad!</h2><p className="mt-4 text-sm leading-relaxed text-muted">Tack, {name.trim()}! Jag går igenom dina designval för {design.company.trim()} och återkommer till <strong className="break-all text-foreground">{email.trim()}</strong>.</p><p className="mt-4 text-sm leading-relaxed text-muted">Nästa steg är ett personligt designförslag för din startsida. Du väljer själv om du vill gå vidare.</p><Link href="/portfolio" className={`${primaryClass} mt-8`}>Se tidigare projekt ↗</Link></div> : <>
              <ol aria-label="Formulärets steg" className="mb-7 grid grid-cols-4 gap-2">{steps.map((label, i) => <li key={label} aria-current={i === step ? 'step' : undefined} className={`text-[10px] ${i === step ? 'text-accent-light' : 'text-muted'}`}><span className={`mb-2 block h-1 rounded-full ${i <= step ? 'bg-accent' : 'bg-foreground/10'}`} />{i + 1}. {label}</li>)}</ol>
              <p className="text-[10px] uppercase tracking-widest text-muted">Steg {step + 1} av 4</p><h2 ref={heading} tabIndex={-1} className="font-display mt-2 text-2xl font-semibold leading-tight">{['Börja med dig.', 'Hitta din känsla.', 'Gör plats för det viktiga.', 'Låt mig göra den personlig.'][step]}</h2>
          ...
          <section aria-label="Liveförhandsvisning" className={`${css.stage} ${mobilePanel !== 'preview' ? css.mobileHidden : ''}`}>
            ...
            <div className="border-t border-foreground/10 px-5 py-4"><div className="flex flex-wrap items-center justify-between gap-3"><p className="text-[10px] text-muted">{industry.label} <span className="mx-1 text-foreground/20">/</span> {selectedStyle.name} <span className="mx-1 text-foreground/20">/</span> {palette.name}</p><button type="button" onClick={share} className="text-xs font-medium text-accent-light hover:text-foreground">Kopiera designlänk ↗</button></div><p className="mt-2 text-[10px] leading-relaxed text-muted">Stilexempel med exempeltexter och inspirationsbilder. Knappar och formulär i exemplet är illustrationer. Delningslänken innehåller företagsnamn och designval, inga kontaktuppgifter.</p><p role="status" className="mt-2 text-xs text-accent-light">{shareMessage}</p>{shareFallback && <label className="mt-2 block text-xs">Din designlänk<input readOnly value={shareFallback} onFocus={e => e.target.select()} className={inputClass} /></label>}</div>
          </section>
        </div>
      ...
      <dialog ref={dialog} className={css.dialog} aria-label="Stor förhandsvisning" onCancel={closePreview} onClose={() => { document.body.style.overflow = previousOverflow.current; }} onClick={event => { if (event.target === event.currentTarget) closePreview(); }}><div className={css.toolbar}><span className="text-sm">Din hemsida, på lite närmare håll.</span><div className="flex items-center gap-3">{deviceButtons}<button type="button" onClick={closePreview} className={subtleButton} autoFocus>Stäng ×</button></div></div><div className={css.canvas}><DemoPreview design={design} mobile={mobile} /></div><p className="px-5 py-3 text-[10px] text-muted">Förhandsvisning av designriktning. Personligt innehåll och funktioner tas fram i nästa steg.</p></dialog>
    </div>
  );
}
