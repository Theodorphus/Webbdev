...
const inputClass = 'mt-2 w-full rounded-[14px] border border-white/15 bg-white/[0.04] px-5 py-4 text-base text-white placeholder:text-white/35 focus:border-accent-light';
...
export default function ContactForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/contact', {
        ...
        body: JSON.stringify({ name: String(data.get('name') ?? '').trim(), email: String(data.get('email') ?? '').trim(), message: String(data.get('message') ?? '').trim(), company: data.get('company') ?? '' }),
      });
  ...
  return <form onSubmit={submit} onFocusCapture={() => { if (!started.current) { started.current = true; trackConversion('contact_started'); } }} className="mt-10 text-left" aria-label={lang === 'sv' ? 'Kontaktformulär' : 'Contact form'}>
    <p className="mb-5 text-xs text-white/55">{lang === 'sv' ? 'Fält markerade med * är obligatoriska.' : 'Fields marked * are required.'}</p>
    <fieldset disabled={status === 'loading'} className="space-y-5"><legend className="sr-only">{t.kontakt.rubrik}</legend>
      ...
      <button type="submit" className="w-full rounded-full bg-accent px-6 py-4 text-base font-semibold text-white hover:bg-[#7773ff] disabled:cursor-wait disabled:opacity-60">{status === 'loading' ? t.kontakt.skickar : t.kontakt.skicka} ↗</button>
    </fieldset>
    <p className="mt-4 text-center text-xs text-white/60">{t.kontakt.risk}</p>
  </form>;
}
