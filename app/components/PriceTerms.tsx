...
export default function PriceTerms() {
  ...
  ].map(([title, text]) => <div key={title}><h3 className="text-sm font-semibold">{title}</h3><p className="mt-3 text-sm leading-relaxed text-muted">{text}</p></div>)}</div></section>;
}
