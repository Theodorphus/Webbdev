...
export default async function CustomerCasePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="mx-auto max-w-[80rem] px-5 pb-24 pt-28 sm:px-8">
    ...
    <div className="mb-12 mt-10 max-w-4xl"><p className="font-mono text-xs uppercase tracking-widest text-accent-light">{item.category}</p><p className="mt-5 text-lg text-muted">{item.name}</p><h1 className="font-display mt-3 text-[clamp(32px,4.6vw,60px)] font-bold leading-[1.08]">{item.title}</h1><p className="mt-6 max-w-3xl text-base leading-relaxed text-muted">{item.intro}</p></div>
    ...
    <Link href={`/portfolio/${cement ? 'wildkull' : 'ockerocement'}`} className="mt-8 inline-block text-sm text-muted hover:text-foreground">Nästa kundcase: {cement ? 'Wildkull Payroll' : 'Öckerö Cementgjuteri'} →</Link>
  </main>;
}
