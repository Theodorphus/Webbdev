...
export default async function CustomerCasePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="mx-auto max-w-[80rem] px-5 pb-24 pt-28 sm:px-8">
    <Link href="/portfolio" className="text-sm text-white/60 hover:text-white">← Alla projekt</Link>
    <div className="mb-12 mt-10 max-w-4xl"><p className="font-mono text-xs uppercase tracking-widest text-accent-light">{item.category}</p><p className="mt-5 text-lg text-white/65">{item.name}</p><h1 className="font-display mt-3 text-[clamp(32px,4.6vw,60px)] font-bold leading-[1.08]">{item.title}</h1><p className="mt-6 max-w-3xl text-base leading-relaxed text-white/65">{item.intro}</p></div>
    {cement ? <BeforeAfter before="/work/cases/cement-before-home.webp" after="/work/cases/cement-after-home.webp" label="Öckerö Cementgjuteris startsida" /> : <div className="max-h-[720px] overflow-y-auto rounded-2xl border border-white/10"><Image src={item.image} alt="Wildkull Payrolls webbplats med tjänster, mötesbokning och grönt bildspråk" width={item.imageWidth} height={item.imageHeight} sizes="(max-width: 1280px) 95vw, 1200px" className="h-auto w-full" priority /></div>}
    <div className="mt-14 grid gap-8 md:grid-cols-3">{[["Designfokus", item.focus], [cement ? 'Utgångsläge' : 'Lösningen', item.before ?? item.solution], [cement ? 'Lösningen' : 'Det besökaren möter', cement ? item.solution : item.outcomes.join(' ')]].map(([title, text]) => <section key={title}><h2 className="font-display text-xl font-semibold">{title}</h2><p className="mt-4 text-sm leading-relaxed text-white/60">{text}</p></section>)}</div>
    {cement && <div className="mt-16 space-y-14">{gallery.map(pair => <section key={pair.title}><h2 className="font-display text-2xl font-semibold">{pair.title}</h2><p className="mb-6 mt-3 text-sm text-white/60">{pair.text}</p><div className="grid gap-5 md:grid-cols-2">{(['before', 'after'] as const).map(version => <figure key={version} className="overflow-hidden rounded-xl border border-white/10"><figcaption className="bg-white/5 px-4 py-3 text-xs text-white/65">{version === 'before' ? 'Tidigare webbplats' : 'Ny webbplats'}</figcaption><a href={`/work/cases/${pair[version]}.webp`} target="_blank" rel="noopener noreferrer" aria-label={`${version === 'before' ? 'Före' : 'Efter'}: ${pair.title}. Öppna bild i ny flik`}><Image src={`/work/cases/${pair[version]}.webp`} alt={`${version === 'before' ? 'Tidigare' : 'Ny'} webbplats: ${pair.title}`} width={1600} height={750} sizes="(max-width: 768px) 95vw, 46vw" className="h-auto w-full" /></a></figure>)}</div></section>)}</div>}
    {!cement && review && <figure className="mt-14 rounded-2xl border border-accent/30 bg-accent/5 p-8 sm:p-12"><p className="text-xs uppercase tracking-widest text-accent-light">Kundens egna ord</p><blockquote className="font-display mt-5 max-w-3xl text-xl leading-relaxed">“{review.text}”</blockquote><figcaption className="mt-5 text-sm text-white/60">{review.namn} · Kundrecension återgiven från webbplatsens recensioner</figcaption></figure>}
    <section className="mt-14 border-t border-white/10 pt-8"><h2 className="font-display text-2xl font-semibold">Det konkreta resultatet i designen</h2><ul className="mt-5 space-y-3 text-sm text-white/75">{item.outcomes.map(text => <li key={text}>✓ {text}</li>)}</ul><p className="mt-6 text-xs leading-relaxed text-white/45">{item.note}</p><a href={item.url} target="_blank" rel="noopener noreferrer" className="mt-5 inline-block text-sm text-accent-light hover:text-white">Öppna projektets webbplats ↗</a></section>
    <CaseCta slug={item.slug} />
    <Link href={`/portfolio/${cement ? 'wildkull' : 'ockerocement'}`} className="mt-8 inline-block text-sm text-white/60 hover:text-white">Nästa kundcase: {cement ? 'Wildkull Payroll' : 'Öckerö Cementgjuteri'} →</Link>
  </main>;
}
